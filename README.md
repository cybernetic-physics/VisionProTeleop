...
**Use the app built from this fork and install its Python package from source.** The upstream App Store app and PyPI package are separate distributions; installing those alone does not install this controller integration. This fork identifies its SDK as `avp_stream` 2.52.0; no PyPI release is implied.
...
**Wuji glove tracking:** build 20 combines glove finger skeletons with controller-based wrist placement, including separate manual wrist mounting offsets and capture-time pose interpolation. See [Wuji glove setup and calibration](docs/wuji_glove_tracking.md).
...
`pose_head` is a 4×4 transform relative to the **current headset position and orientation**: X right, Y up, −Z forward, in meters. It is calculated as `inverse(head_world) @ world_alignment @ controller_local @ grip_offset`. A manual calibration panel opens on startup: adjust both controllers together, then each grip, preview in passthrough, and save. The saved corrections are shared by headset overlays, recordings, and this Python SDK. Review alignment after restarting or recentering tracking; no automatic hand snapping is performed. This is not a fixed initial head root or a yaw-only body root.
...
### Wuji hands from Vision Pro optical tracking

Use **Use Vision Pro hands only** in build 24, then run the [optical-hand bridge](docs/wuji_optical_hands.md) on Thor. Left-only operation is supported; no glove or controller is required.
